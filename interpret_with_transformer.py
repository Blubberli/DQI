...
def f(x):
    tv = torch.tensor([tokenizer.encode(v, padding=True, max_length=512, truncation=True) for v in x])
...
def get_attributions(path_to_data, model, tokenizer, dimension, output_path):
    ...
    print(data.columns)
    comments = data.cleaned_comment.values[6:]
    true_labels = data[dimension].values[6:]
...
# for each point in the test set: collect the attributions, aggregate attributions for the words, show top 10?
# keep track of examples with high attributions for each class
