...
from multitask_model import RobertaMultiTask
...
def run_train_with_trainer(train_data, dev_data, test_data, data_args, model_args, training_args,
    ...
    dev_data.dataset['predictions'] = F.softmax(torch.tensor(dev_predictions.predictions[0]), dim=-1).tolist()
    dev_data.dataset.to_csv(f'{str(split_dir)}/dev_df_with_predictions.csv', index=False, sep="\t")
    test_data.dataset['predictions'] = F.softmax(torch.tensor(test_predictions.predictions[0]), dim=-1).tolist()
...
if __name__ == '__main__':
    ...
    config_task1 = RobertaConfig.from_pretrained(
        'roberta-base',
        num_labels=model_args.task1_labels_num,
    )
    config_task2 = RobertaConfig.from_pretrained(
        'roberta-base',
        num_labels=model_args.task2_labels_num,  # The number of output labels--3 for classification.
    )
    config_task3 = RobertaConfig.from_pretrained(
        'roberta-base',
        num_labels=model_args.task3_labels_num,  # The number of output labels--3 for classification.
    )
    config_task4 = RobertaConfig.from_pretrained(
        'roberta-base',
        num_labels=model_args.task4_labels_num,  # The number of output labels--3 for classification.
    )
    model = RobertaMultiTask(roberta_config_task1=config_task1, roberta_config_task2=config_task2,
                             roberta_config_task3=config_task3, roberta_config_task4=config_task4)
    print("loaded model of type %s" % str(type(model)))
    dev_reports = []
    test_reports = []
    for i in range(0, 5):
        train, dev, test = get_datasets(data_args=data_args, i=i)
